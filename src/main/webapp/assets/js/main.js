(function() {
    ...
    document.querySelectorAll('.donut').forEach(el => {
        // data-percent 값을 읽어 CSS 변수 --percent에 설정
    ...
    function renderCalendar(year, month1to12) {
        ...
        for (let d = 1; d <= lengthOfMonth; d++) {
            ...
            // 클릭 핸들러 (필요 시 주석 해제)
            // btn.addEventListener("click", () => { /* 출석/메모 로직 */ });
    ...
    // 출석 데이터 가져오기 (더미 로직 포함)
    function getAttendanceDay() {
        // APP_CTX가 정의되지 않은 경우 (Canvas/로컬 HTML 환경), 더미 데이터 사용
        if (typeof APP_CTX === 'undefined') {
            const dummyDays = [4, 7, 10, 15]; // 임시 출석 날짜
            applyAttendance(dummyDays);
            return;
        }
