...
public class FindPwService implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		MemberVO mvo = MemberVO.builder().memberId(memberId).email(email).build();
		...
		if (memberVO != null) {
			...
			if (memberId.startsWith("google_")) {
				...
				String html = """
						...
						""".formatted(
						"http://localhost:8090" + request.getContextPath() + "/GopwUpdate.do?memberId=" + memberId);
			...
			payload.put("ok", false);
			payload.put("message", "비밀번호 찾기 실패\n입력한 아이디와 이메일을 확인해주세요");
		}
